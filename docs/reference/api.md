...
- `GET /api/terminals/:terminalId/screen?lines=40&raw=1`: `lines` defaults to 40 and must be an integer from 1–200. Omit `raw=1` for processed text. Returns `{ "text": "...", "savedAt": null, "raw": true }`. With no live session, reads `<stateDir>/state/transcripts/<terminalId>.screen.txt`; `savedAt` is its modification time in ISO format and `raw` is `false`. At most 200 processed lines are saved best-effort on all normal session teardown paths; abrupt server termination cannot guarantee a save. Returns `404` without a screen and `400` for invalid line counts.
