...
Hub 用一个 API 进程服务所有已登记的项目，而不是每个项目各跑一个服务器。每个项目有自己的上下文（终端运行时、监控、代码洞察存储、提示词、用量缓存），在第一次被请求点名时创建，并挂载在 `/api/p/<key>/` 之下；`Host`、`Origin` 与令牌校验在 hub 层统一执行一次，之后请求才会到达项目。项目内的代理拿到的是属于该项目的 API 地址 `<hub>/api/p/<projectId>`，它出现在 Claude 钩子命令、带守卫的用户级 Codex 钩子以及 `OCTOGENT_API_BASE` 中，因此即便不同项目的终端 id 会重复，钩子和工作者内的 CLI 也总能落到拥有该会话的项目。单项目服务器是同一套代码：只在根路径挂载一个上下文，地址不带前缀。Hub 只写一份服务器日志 `~/.octogent/hub/logs/server.log`，每个项目的日志行以 `[<slug>]` 标注。`octogent hub start` 启动它，任何需要服务器的 CLI 命令也会按需自动启动它；见[运行 hub](../reference/cli.md#运行-hub)。
