...
如果 [hub](#运行-hub) 已在运行并服务当前项目，直接运行 `octogent` 不会为同一个项目再启动第二个服务器，而是打印该项目在 hub 下的地址 `<hub>/p/<slug>/`，并给出两条路：继续使用 hub（项目里运行的 CLI 命令已经会连到它），或运行 `octogent --standalone`——它总是像以前一样启动单项目服务器（hub 占着 `8787` 时顺延到下一个空闲端口）。Web UI 目前还不认 `/p/<slug>/` 前缀，所以工作面板本身暂时仍从单项目服务器使用。
