...
Hub 是一个 API 进程同时服务 `~/.octogent/projects.json` 中登记的所有项目；单项目服务器就是只在根路径挂载一个项目的 hub，因此本页所有路径在单项目模式下原样可用。在 hub 下，这些路由（HTTP 与 WebSocket 均是）都位于项目前缀之后：`/api/p/<key>/api/...`，其中 `<key>` 可以是项目 id、slug，或改名后保留为别名的旧 slug；项目在第一次被请求点名时才加载；在 `OCTOGENT_HUB_PROJECT_IDLE_MS`（默认 30 分钟；`0` 表示从不）时长内既没有 PTY 会话、没有打开的终端事件 WebSocket、也没有进行中的请求时，它会被卸载，直到下一次请求重新加载它；未知的 key 返回 `404`。Claude 钩子命令、用户级 Codex 钩子里的守卫条件，以及每个终端环境中的 `OCTOGENT_API_BASE` 都使用 `<hub>/api/p/<projectId>`（用 id 而不是 slug，因为改名会改变 slug），终端还会拿到 `OCTOGENT_PROJECT_ID`。Hub 自身的路由有：`GET /api/hub/health`（下文的健康快照，计数为所有已加载项目之和，另含 `hub.json` 记录的 hub 构建信息，即 `version`，以及已知时的 `commit` 与 `builtAt`，然后是 `pid`、`maxTerminalSessions`、`projects: { registered, loaded }` 与 `loadedProjects`）；`GET /api/projects`（`{ "projects": [{ id, name, slug, path, loaded, summary? }] }`，已加载项目的 `summary` 含会话数、终端计数与 `lastActivityAt`）；`POST /api/projects`，请求体 `{ "path": "/absolute/dir", "name": "可选" }`，为该目录生成 `.octogent/` 骨架并登记（新登记返回 `201`，已登记返回 `200`，路径不是已存在的绝对目录时返回 `400`；仅限回环地址或持有访问令牌的调用方）。`GET /p/<key>/...` 提供 Web 应用；Web 应用根据自身地址推导 API 前缀：在 `/p/<key>/...` 页面上，所有请求与 WebSocket 都发往 `/api/p/<key>/api/...`，其他页面则发往 `/api/...`；在 `/` 上它先请求 `GET /api/projects`，有应答就显示项目总览，返回 `404` 则显示单项目应用。每个项目默认最多 12 个 PTY 会话（可用 `OCTOGENT_MAX_TERMINAL_SESSIONS` 覆盖），整个 hub 合计最多 32 个（`OCTOGENT_HUB_MAX_TERMINAL_SESSIONS`）；超过 hub 上限再创建终端返回 `429`。
...
`POST /api/terminals` 接受 `inheritEnv`（最多 64 个变量名，须匹配 `^[A-Z_][A-Z0-9_]*$`）以及 `env`（恰好包含这些变量名的对象，值为不含 NUL 与换行的字符串）；这些值会叠加在项目 `.octogent/env` 之上，用于该终端的会话。两个字段不一致时返回 `400`。记录与快照只以 `inheritedEnv` 保存变量名；变量值只保存在内存中，不会持久化。
